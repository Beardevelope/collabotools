...
export class WorkspacesService {
    ...
    async createWorkspace(userId: number, createworkspaceDto: CreateWorkspaceDto) {
        ...
        try {
            ...
            const newWorkSapceId = await queryRunner.manager
                .createQueryBuilder()
                .select(['MAX(workspaces_model.id) AS max_id'])
    ...
    async findAllWorkspace(userId: number) {
        const workspaces = await this.workspaceRepository
            .createQueryBuilder('workspaces_model')
            .select(['workspaces_model.id', 'workspaces_model.name'])
            .where(
                'workspaces_model.id IN (SELECT uww.workspacesModelId FROM users_model_workspaces_workspaces_model uww WHERE uww.usersModelId = :userId)',
                { userId },
            )
            .getMany();
    ...
    async deleteWorkspace(userId: number, workspaceId: number) {
        ...
        try {
            ...
            await queryRunner.manager.delete('users_model_workspaces_workspaces_model', {
    ...
    async inviteMembers(userId: number, workspaceId: number, membersDto: MembersDto) {
        ...
        const values = await Promise.all(
            members.map(async (memberId) => {
                ...
                const isUserInWorkSpaceMembers = isSpaceMembers.some(
                ...
                if (isUserInWorkSpaceMembers) {
                    ...
                    throw new NotFoundException(
                        `사용자 ID ${memberId} (이)가 워크스페이스 멤버로 이미 있습니다.`,
                    );
                }
    ...
    private async findByUserId(userId: number) {
        ...
        try {
            const user = await queryRunner.manager
                .createQueryBuilder()
                .select(['id AS user_id', 'email AS email', 'name AS name'])
            ...
            if (error instanceof HttpException) {
                ...
                if (error.name === 'TypeError')
                    throw new NotFoundException(
                        `가입 안된 사용자 ID (userId:${userId})가 있습니다.`,
                    );
    ...
    private async listWorkSpaceMember(workspaceId: number) {
        ...
        try {
            const user = await queryRunner.manager
                .createQueryBuilder()
                .select(['usersModelId AS user_id'])
